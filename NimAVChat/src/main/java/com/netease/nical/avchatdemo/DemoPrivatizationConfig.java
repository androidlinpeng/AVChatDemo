...
public class DemoPrivatizationConfig {
    ...
    // 音视频加密
    private static final String KEY_NRTC_STREAM_ENC_TYPE = "nrtc_stream_enc_type";
    private static final String KEY_NRTC_STREAM_ENC_KEY = "nrtc_stream_enc_key";
    ...
    private static ServerAddresses parseAddresses(JSONObject jsonObject) {
        ...
        addresses.nrtcStreamEncType = jsonObject.optInt(KEY_NRTC_STREAM_ENC_TYPE, 0);
        addresses.nrtcStreamEncKey = jsonObject.optString(KEY_NRTC_STREAM_ENC_KEY);
    ...
    private static void checkValid(ServerAddresses addresses) {
        ...
        //配置走1-1协议，但是加密方式不是RSA+RC4，或没有配置module
        if (addresses.handshakeType == NimHandshakeType.V1 && addresses.module == null) {
            throw new IllegalArgumentException("ServerAddresses handshakeType is V1, but module is null");
        }

        if (addresses.handshakeType == NimHandshakeType.V0 && (addresses.negoKeyEncaKeyParta == null || addresses.negoKeyEncaKeyPartb == null)) {
            throw new IllegalArgumentException("ServerAddresses handshakeType is V0, but negoKeyEncaKeyParta is not null or negoKeyEncaKeyPartb  is null");
        }
    }
    ...
    private static void autoAdjust(@NonNull ServerAddresses addresses) {
        ...
        addresses.nrtcStreamEncKey = TextUtils.isEmpty(addresses.nrtcStreamEncKey) ? null : addresses.nrtcStreamEncKey;
